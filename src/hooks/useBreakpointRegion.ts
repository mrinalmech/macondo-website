...
for (const bp in breakpoints) {
  breakpointsNumbers[bp as Breakpoint] = parseInt(breakpoints[bp as Breakpoint].slice(0, -2));
}
...
export function useBreakpointRegion() {
  const [breakpointRegion, setBreakpointRegion] = useState(null as BreakpointRegion);
